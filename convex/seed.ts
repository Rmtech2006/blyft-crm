...
export const run = mutation({
  ...
  handler: async (ctx) => {
    ...
    // ── Message Templates ─────────────────────────────────────────────────────
    // ── BLYFT Onboarding Email Templates (PDF) ──────────────────────────────
    const blyftTemplates: Array<{
      title: string
      category: "CLIENT_COMMS" | "LEAD_FOLLOWUP" | "INTERNAL" | "FINANCE" | "SOCIAL" | "PROPOSAL"
      content: string
      variables: string[]
    }> = [
      {
    ...
    for (const t of blyftTemplates) {
      await ctx.db.insert("messageTemplates", { ...t, isLocked: true, usageCount: 0 });
    }
